...
class TokenLearnerModuleV11(nn.Module):
    ...
    def __init__(self, in_channels, num_tokens, num_groups, dropout_rate=0.2): # dropout_rate的值是0.代表不使用dropout。
        ...
        self.attention_maps = nn.Sequential(
            nn.Conv1d(self.in_channels, self.in_channels, kernel_size=1, stride=1, padding=0, groups=self.num_groups, bias=False),
            nn.Conv1d(self.in_channels, self.num_tokens, kernel_size=1, stride=1, padding=0, bias=False),
        )
    ...
    def forward(self, inputs):
        ...
        selected = inputs # selected的shape是[batch,mem_size+special_num_token,dim]
        selected = self.norm(selected) # 归一化层，对输入张量的channels维度进行归一化，in_channels是输入通道数。
        selected = selected.permute(0, 2, 1) # Shape:  [bs, dim, mem_size+special_num_token]
        selected = self.attention_maps(selected) # Shape: [bs, n_token, mem_size+special_num_token].
        selected = F.softmax(selected, dim=-1) # Shape: [bs, n_token, mem_size+special_num_token]. 
        ...
        outputs = torch.einsum("...si,...id->...sd",  selected, feat) # outputs的shape是[batch,n_token,dim]
