...
class TokenLearnerModuleV11(nn.Module):
    ...
    def __init__(self, in_channels, num_tokens, num_groups, dropout_rate=0.4): # dropout_rate的值是0.代表不使用dropout。
        """Applies learnable tokenization to the 2D inputs.
        ...
        self.attention_maps = nn.Sequential( # 对应源码的attention_layers.MlpBlock
    ...
    def forward(self, inputs):
        ...
        selected = selected.permute(0, 2, 1)  # Shape: [bs, h*w, n_token].即[batch,dim,mem_size+special_num_token]
        selected = F.softmax(selected, dim=-1) # selected的shape是[batch,mem_size+special_num_token,dim]
