...
class PreProcess(nn.Module): ### 输入 batch step 1 28 28   变换到  batch step len dim  方便进行时序处理     #我觉得是没加cls
    """定义了一个数据预处理层，
    它包括一个卷积层，
...
class TokenLearnerMHA(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.query = nn.Parameter(torch.randn(batch, num_tokens, dim).cuda()) # Parameter是一种特殊的Tensor，会被自动添加到模型的参数列表中
        self.attn = nn.MultiheadAttention(dim, 8, 0.1, batch_first = True) # batch_first=True表示输入数据的形状为(batch, step, token_num, dim)，0.1是dropout
...
class TokenTuringMachineUnit(nn.Module):
    def __init__(self) -> None:
        ...
        self.TokenLearner1 = TokenLearnerModuleV11(in_channels=64,num_tokens=27,num_groups=1) 
        self.TokenLearner2 = TokenLearnerModuleV11(in_channels=64,num_tokens=88,num_groups=1) 
        self.TokenLearnerMHA = TokenLearnerMHA()
        self.TokenAddEraseWrite = TokenAddEraseWrite()
        self.transformer_block = nn.TransformerEncoderLayer(d_model = dim, nhead = 8, dim_feedforward = dim * 3, dropout = 0.)
    ...
    def forward(self,memory_tokens,input_tokens):
        all_tokens=torch.cat((memory_tokens,input_tokens),dim=1) # all_token的shape是[batch,mem_size+special_num_token,dim]

        # 读之前也要加一个位置嵌入
        if self.use_positional_embedding:
            all_tokens = all_tokens.cuda()
            posemb_init = torch.nn.Parameter(torch.empty(1, all_tokens.size(1), all_tokens.size(2))).cuda()
            init.normal_(posemb_init, std=0.02)
            all_tokens = all_tokens + posemb_init # mem_out_tokens的shape是[batch,mem_size+special_num_token,dim]
...
# 在需要添加位置嵌入的地方
        if self.use_positional_embedding:
            ...
            init.normal_(posemb_init, std=0.02)
            mem_out_tokens = mem_out_tokens + posemb_init # mem_out_tokens的shape是[batch,mem_size+special_num_token,dim]
            # posemb_init的shape是[1,mem_size+special_num_token,dim]
        ...
        if self.memory_mode == 'TL':
            ...
            mem_out_tokens = self.TokenAddEraseWrite(memory_tokens,output_tokens)
...
class TokenTuringMachineEncoder(nn.Module):
    ...
    def forward(self, input, mem=None):
        ...
        outs=[]
        if mem == None:
            mem = torch.zeros(b,self.mem_size,c).cuda() #  c, h, w
        else:
            mem = self.mem
        for i in range(t):
